...
def publicacionImpactante(publicaciones: List[Publicacion], minLongitud: Int): PublicacionConPuntaje = {
  ...
  // 1) Transformar cada lista de reacciones en su puntaje total:
  val conPuntaje =
    filtradas.map { p =>
      val listaPuntajes = p.reacciones.map(r => puntajeReaccion(r))
      val total = listaPuntajes.sum
      PublicacionConPuntaje(p, total)
    }
  conPuntaje.maxBy(_.puntaje)
}
  // 2) Obtener la publicación con mayor puntaje
  val mejor = publicacionImpactante(publicaciones, 40)
  mejor
